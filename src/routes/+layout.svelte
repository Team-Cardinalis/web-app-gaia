<script lang="ts">
	...
	import { t, setLocale, currentLocale } from '$lib/translations/index';
	...
	>
		<nav class="flex-1">
			<ul class="flex h-full w-full flex-row items-center gap-x-2 px-2 md:flex-col md:justify-start md:gap-y-2 md:gap-x-0 md:px-0">
				<NavItem href="/" imgSrc="/favicon.svg" alt={t('common.home')} active={$page.url.pathname === '/' } />
				<NavItem href="/equipments" imgSrc="/equipment.svg" alt={t('common.equipments')} active={$page.url.pathname === '/equipments' } />
				<NavItem href="/settings" imgSrc="/settings.svg" alt={t('common.settings')} active={$page.url.pathname === '/settings' } />
			</ul>
		</nav>
	</aside>
